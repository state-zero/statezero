...
from modelsync.core.event_emitters import (ConsoleEventEmitter,
                                           PusherEventEmitter)
from modelsync.core.types import ActionType, ORMModel
...
class DjangoPusherEventEmitter(PusherEventEmitter):
    def __init__(
        ...
        """
        app_id = app_id or settings.MODELSYNC_PUSHER.get("APP_ID")
        key = key or settings.MODELSYNC_PUSHER.get("KEY")
        secret = secret or settings.MODELSYNC_PUSHER.get("SECRET")
        cluster = cluster or settings.MODELSYNC_PUSHER.get("CLUSTER")
