...
class ModelSyncViewAccessGate(BasePermission):
    """
    ...
    def has_permission(self, request, view):
        ...
        if request.path.startswith("/modelsync/"):
            logger.debug("Path matches modelsync endpoints. DEBUG=%s", settings.DEBUG)
            if settings.DEBUG:
                # In development mode, try to set a default user if one is provided.
                default_user_func_path = getattr(
                    settings, "MODELSYNC_DEFAULT_USER_FUNC", None
                )
        ...
        logger.debug(
            "Path does not match modelsync endpoints; delegating permission check to %s",
            self.view_access.__class__.__name__,
        )
        permission = self.view_access.has_permission(request, view)
        logger.debug("Non-modelsync endpoint permission result: %s", permission)
        return permission
